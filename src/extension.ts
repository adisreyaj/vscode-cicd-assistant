...
import { createDockerfile } from './helpers/docker.helper';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  let createSonarFile = vscode.commands.registerCommand(
  ...
  context.subscriptions.push(createSonarFile);

  let createDockerCommand = vscode.commands.registerCommand(
    'extension.createDockerfile',
    () => {
      const typeSelection = vscode.window.createQuickPick();
      typeSelection.placeholder = 'Select your Project type?';
      typeSelection.items = [
        {
          label: 'Angular',
          detail: 'Create Dockerfile for Angular project',
        },
        {
          label: 'Node',
          detail: 'Create Dockerfile for Node(TS Based) project',
        },
      ];
      typeSelection.show();
      typeSelection.onDidChangeSelection(async (data) => {
        if (data[0].label === 'Angular') {
          await createDockerfile('Angular');
        } else if (data[0].label === 'Node') {
          await createDockerfile('Node');
        }
        typeSelection.hide();
      });
    },
  );
  context.subscriptions.push(createDockerCommand);
}
